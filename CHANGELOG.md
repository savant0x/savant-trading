...
- **FID-227** — Anvil testnet bot dead (0 trades after 68+ cycles). Three stacked bottlenecks: (1) `[trading.funnel_v1]` block missing from `test-anvil.toml` → funnel pre-scorer never runs; (2) `hunt_mode_bypass=true` forces `output_K=0` even with funnel enabled — no seed path for 0-position case; (3) conviction gate + `MAX_NON_PASS=15` cap truncates 27 illiquid Anvil micro-cap pairs to 0 BUY verdicts.
  - `config/test-anvil.toml`: added `[trading.funnel_v1]` block (`enabled=true, top_k=12, min_score_threshold=0.20`).
  - `src/engine/mod.rs`: Anvil-native bypass — sets `SAVANT_GATE_DISABLED=1` when `scan_all_pairs && hunt_mode && positions.is_empty()`, skipping conviction gate + confidence floor.
  - `src/api/mod.rs`: `MAX_NON_PASS` raised from 15→100 when `SAVANT_GATE_DISABLED` is set.
  - `src/execution/dex_trader.rs`: defensive purge of stale `dex_state.json` positions on Anvil startup (prevents phantom re-ingestion from previous sessions).
