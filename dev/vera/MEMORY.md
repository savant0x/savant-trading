...
## v0.15.7-a.1 — SHIPPED (2026-06-21 03:55)

**Final SHA:** `b53d06bf` (annotated tag `v0.15.7-a.1`).
**Commit message:** `fix(FID-225): decouple per-token threshold + classify phantom positions`.
**Changes:** FID-225 round 1 (threshold decouple \$0.10→\$5.00) + round 2 (phantom-position classification, helper + 5 tests); carried as single coherent commit per Spencer's "Single commit message" directive. Pre-push hook caught one fmt violation which was folded via `--amend --no-edit` into the same commit
