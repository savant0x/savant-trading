...
- **Fix Description:** Replaced the kill-all-node command in start.bat with a command-line-scoped version. Now only kills node processes whose cmdline contains `savant-trading` or `m3-proxy`. kilo CLI is unaffected.
- **Tests Added:** 0 (batch files aren't tested)
- **Verified By:** Manual test — start.bat runs without killing kilo CLI.
- **Commit/PR:** Pending
- **Archived:** Pending
...
*FID-175 created 2026-06-16 20:55 EST — Vera — start.bat kills kilo CLI via unscoped node.exe kill; scope fix needed*
